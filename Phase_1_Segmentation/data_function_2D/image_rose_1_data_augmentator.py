...
def img_rotator_ROSE_SVC(num_aug, angles, image, groundtruth, thickgroundtruth, thingroundtruth):
    ...
    # only conduct rotation when given two distince angle values
        ...
        num_files = image.shape[2]
        # conduct angle rotation 
        out_img = np.zeros((num_row, num_col, num_files*(num_aug+1))) # it's num_aug+1, because I stored the original image + the assigned number of augmentation 
                                                                      # (for example, If I had three augmentation angles, I will have output as [x, y, (num_aug+1)])
        out_gt = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        out_thickGt = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        out_thinGt = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        for i_file in range(num_files):
            ...
            for i_angle in range(num_aug+1):
                ...
                rotated_current_thickgt = cv2.warpAffine(current_thickgt, M, (int(num_row), int(num_col)), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
                rotated_current_thingt = cv2.warpAffine(current_thingt, M, (int(num_row), int(num_col)), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
...
def img_flipper_ROSE_SVC(image, groundtruth, thickgroundtruth, thingroundtruth):
    ...
    num_col = image.shape[1]
    num_files = image.shape[2]
    out_img = np.zeros((num_row, num_col, num_files*(2+1))) # here, I constructed to conduct two flipping (horizontal and vertical). 
                                                            # So in the end, I will have output as [x, y, (2+1)]
    out_gt = np.zeros((num_row, num_col, num_files*(2+1)))   
    out_thickGt = np.zeros((num_row, num_col, num_files*(2+1)))        
    out_thinGt = np.zeros((num_row, num_col, num_files*(2+1)))            
    for i_file in range(num_files):
        # grab out current file
        current_file = converted_image[:, :, i_file]
        current_gt = converted_gt[:, :, i_file]
...
def img_elastic_deformer_ROSE_SVC(image, groundtruth, thickgroundtruth, thingroundtruth):
    ...
    num_col = image.shape[1]
    num_files = image.shape[2]
    out_img = np.zeros((num_row, num_col, num_files*(1+1))) # like in the image flipping, this time, I only conduct one augmentation (elastic deformation).
                                                            # so, its outut as [x, y, (1+1)]
    out_gt = np.zeros((num_row, num_col, num_files*(1+1)))       
    out_thickGt = np.zeros((num_row, num_col, num_files*(1+1)))        
    out_thinGt = np.zeros((num_row, num_col, num_files*(1+1)))        
    for i_file in range(num_files):
        # grab out current file
        current_file = converted_image[:, :, i_file]
        current_gt = converted_gt[:, :, i_file]
...
def img_contrast_jitter_ROSE_SVC(image, groundtruth, thickgroundtruth, thingroundtruth):
    ...
    num_col = image.shape[1]
    num_files = image.shape[2]
    out_img = np.zeros((num_row, num_col, num_files*(2+1))) # like in the image flipping, this time, I conduct two augmentation (higher contrast and lower contrast).
    out_gt = np.zeros((num_row, num_col, num_files*(2+1)))        
    out_thickGt = np.zeros((num_row, num_col, num_files*(2+1)))        
    out_thinGt = np.zeros((num_row, num_col, num_files*(2+1)))        
    for i_file in range(num_files):
