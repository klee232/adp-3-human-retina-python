...
def img_rotator_ROSEO(num_aug, angles, SVC_image, DVC_image, IVC_image, FAZ_groundtruth, junct_groundtruth, vessel_groundtruth):
    ...
    # only conduct rotation when given two distince angle values
        ...
        # conduct angle rotation 
        num_row = SVC_image.shape[0]
        num_col = SVC_image.shape[1]
        num_files = SVC_image.shape[2]
        out_SVC_img = np.zeros((num_row, num_col, num_files*(num_aug+1))) # it's num_aug+1, because I stored the original image + the assigned number of augmentation 
                                                                          # (for example, If I had three augmentation angles, I will have output as [x, y, (num_aug+1)])
        out_DVC_img = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        out_IVC_img = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        out_FAZGt = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        out_junctGt = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
        out_vesselGt = np.zeros((num_row, num_col, num_files*(num_aug+1))) 
...
def img_flipper_ROSEO(SVC_image, DVC_image, IVC_image, FAZ_groundtruth, junct_groundtruth, vessel_groundtruth):
    ...
    num_col = SVC_image.shape[1]
    num_files = SVC_image.shape[2]
    out_SVC_img = np.zeros((num_row, num_col, num_files*(2+1))) # here, I constructed to conduct two flipping (horizontal and vertical). 
                                                                # So in the end, I will have output as [x, y, (2+1)]
    out_DVC_img = np.zeros((num_row, num_col, num_files*(2+1))) 
    out_IVC_img = np.zeros((num_row, num_col, num_files*(2+1))) 
    out_FAZGt = np.zeros((num_row, num_col, num_files*(2+1)))   
    out_junctGt = np.zeros((num_row, num_col, num_files*(2+1)))        
    out_vesselGt = np.zeros((num_row, num_col, num_files*(2+1)))            
    for i_file in range(num_files):
        # grab out current file
        current_SVC_file = converted_SVC_image[:, :, i_file]
        current_DVC_file = converted_DVC_image[:, :, i_file]
...
def img_elastic_deformer_ROSEO(SVC_image, DVC_image, IVC_image, FAZ_groundtruth, junct_groundtruth, vessel_groundtruth):
    ...
    aug = Compose([
    A.ElasticTransform(alpha=40, sigma=6, alpha_affine=0,
                       interpolation=1, border_mode=0, value=0)  # zeros outside
    ])
    num_row = SVC_image.shape[0]
    num_col = SVC_image.shape[1]
    num_files = SVC_image.shape[2]
    out_SVC_img = np.zeros((num_row, num_col, num_files*(1+1))) # like in the image flipping, this time, I only conduct one augmentation (elastic deformation).
                                                                # so, its outut as [x, y, (1+1)]
    out_DVC_img = np.zeros((num_row, num_col, num_files*(1+1))) 
    out_IVC_img = np.zeros((num_row, num_col, num_files*(1+1))) 
    out_FAZGt = np.zeros((num_row, num_col, num_files*(1+1)))       
    out_junctGt = np.zeros((num_row, num_col, num_files*(1+1)))        
    out_vesselGt = np.zeros((num_row, num_col, num_files*(1+1)))        
    for i_file in range(num_files):
        # grab out current file
        current_SVC_file = converted_SVC_image[:, :, i_file]
...
def img_contrast_jitter_ROSEO(SVC_image, DVC_image, IVC_image, FAZ_groundtruth, junct_groundtruth, vessel_groundtruth):
    ...
    num_col = SVC_image.shape[1]
    num_files = SVC_image.shape[2]
    out_SVC_img = np.zeros((num_row, num_col, num_files*(2+1))) # like in the image flipping, this time, I conduct two augmentation (higher contrast and lower contrast).
    out_DVC_img = np.zeros((num_row, num_col, num_files*(2+1))) 
    out_IVC_img = np.zeros((num_row, num_col, num_files*(2+1))) 
    out_FAZGt = np.zeros((num_row, num_col, num_files*(2+1)))        
    out_junctGt = np.zeros((num_row, num_col, num_files*(2+1)))        
