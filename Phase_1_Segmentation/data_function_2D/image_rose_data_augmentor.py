    ...
    # only conduct rotation when given two distince angle values
        ...
        for i_file in range(num_files):
            ...
            for i_angle in range(num_aug+1):
                ...
                rotated_current_file = cv2.warpAffine(current_file, M, (num_row/2.0, num_col/2.0), flags=cv2.INTER_LINAEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
                rotated_current_gt = cv2.warpAffine(current_gt, M, (num_row/2.0, num_col/2.0), flags=cv2.INTER_LINAEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    ...
    for i_file in range(num_files):
        ...
        for i_aug in range(2+1):
            if i_aug == 0:
                ...
                flipped_current_file = np.flip(current_file, axis=i_aug)
                flipped_current_gt = np.flip(current_gt, axis=i_aug)
...
def img_contrast_jitter(image, groundtruth):
    ...
    for i_file in range(num_file):
        current_file = converted_image[:, :, i_file]
        current_gt = convert_gt[:, :, i_file]
        enhancer = ImageEnhance.Contrast(current_file)
        for i_aug in range(2+1):
            if i_aug == 0:
                out_img[:, :, i_aug+(i_file)*(2+1)] = current_file
            elif i_aug == 1:
                out_img[:, :, i_aug+(i_file)*(2+1)] = enhancer.enhance(1.5)
            else:
                out_img[:, :, i_aug+(i_file)*(2+1)] = enhancer.enhance(0.8)
